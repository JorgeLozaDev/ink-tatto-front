...
export const filterMettings = async (endpoint, token, data) => {
  const headers = {
    Authorization: "Bearer " + token.credentials,
  };
...
export const getAllUsers = async (endpoint, token,data) => {
  const headers = {
    Authorization: "Bearer " + token.credentials,
  };
  const allUsers = await axios.post(`${URL}${endpoint}`,data, { headers });
  return allUsers;
};
